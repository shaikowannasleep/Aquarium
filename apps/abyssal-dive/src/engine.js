...
function defaultParams() {
  return {
    ...
    // senses
    rFlee: 132, rLure: 210, lureCore: 15,
  ...
  step(dt, world) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let q = 0; q < obs.length; q++) {
        const O = obs[q];
        if (O.passable) continue;   // kelp: fish swim straight through
      ...
      if (L && L.active) {
        ...
        if (d < R && d > 1e-4) {
           ...
           * The core must stay SMALLER than a refuge mouth, otherwise the
           * lamp shoves the school back out of the very crevice it is
           * being led into. */
          const t = 1 - d / R;
          const core = d < p.lureCore ? -0.8 : 1;
      ...
      const sc = this.speedScale[i] || 1;
      const vmax = p.maxSpeed * sc * lerp(1, p.panicSpeedBoost, this.stress[i]);
      const vmin = p.minSpeed * sc;
      const nsp = Math.hypot(nvx, nvy);
      if (nsp > vmax) { const k = vmax / nsp; nvx *= k; nvy *= k; }
      else if (nsp < vmin) {
        const k = vmin / (nsp || 1e-6); nvx *= k; nvy *= k;
      }
